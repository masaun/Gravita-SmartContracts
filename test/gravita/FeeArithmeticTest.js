...
contract("Fee arithmetic tests", async accounts => {
  ...
  base rate */
	...
	it("decayBaseRateFromBorrowing(): returns the initial base rate for no time increase", async () => {
		await vesselManagerTester.setBaseRate(ZERO_ADDRESS, dec(5, 17))
		await vesselManagerTester.setLastFeeOpTimeToNow(ZERO_ADDRESS)

		const baseRateBefore = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		assert.equal(baseRateBefore, dec(5, 17))

		await vesselManagerTester.unprotectedDecayBaseRateFromBorrowing(ZERO_ADDRESS)
		const baseRateAfter = await vesselManagerTester.baseRate(ZERO_ADDRESS)
	...
	it("decayBaseRateFromBorrowing(): returns the initial base rate for less than one minute passed ", async () => {
		...
		// 1 second
		const baseRateBefore_1 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		assert.equal(baseRateBefore_1, dec(5, 17))

		await th.fastForwardTime(1, web3.currentProvider)

		await vesselManagerTester.unprotectedDecayBaseRateFromBorrowing(ZERO_ADDRESS)
		const baseRateAfter_1 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		...
		const baseRateBefore_2 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		await th.fastForwardTime(17, web3.currentProvider)

		await vesselManagerTester.unprotectedDecayBaseRateFromBorrowing(ZERO_ADDRESS)
		const baseRateAfter_2 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		...
		const baseRateBefore_3 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		await th.fastForwardTime(29, web3.currentProvider)

		await vesselManagerTester.unprotectedDecayBaseRateFromBorrowing(ZERO_ADDRESS)
		const baseRateAfter_3 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		...
		const baseRateBefore_4 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
		await th.fastForwardTime(50, web3.currentProvider)

		await vesselManagerTester.unprotectedDecayBaseRateFromBorrowing(ZERO_ADDRESS)
		const baseRateAfter_4 = await vesselManagerTester.baseRate(ZERO_ADDRESS)
	...
	it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.01", async () => {
		...
		for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
			// Set base rate to 0.01 in VesselManager
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, dec(1, 16))
	...
	it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.1", async () => {
		// baseRate = 0.1
		for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
			// Set base rate to 0.1 in VesselManager
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, dec(1, 17))
			const contractBaseRate = await vesselManagerTester.baseRate(ZERO_ADDRESS)
	...
	it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.34539284", async () => {
		...
		for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
			// Set base rate to 0.1 in VesselManager
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, "345392840000000000")
			const contractBaseRate = await vesselManagerTester.baseRate(ZERO_ADDRESS)
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, "345392840000000000")
	...
	it("decayBaseRateFromBorrowing(): returns correctly decayed base rate, for various durations. Initial baseRate = 0.9976", async () => {
		// baseRate = 0.9976
		for (i = 0; i < decayBaseRateResults.seconds.length; i++) {
			// Set base rate to 0.9976 in VesselManager
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, "997600000000000000")
			await vesselManagerTester.setBaseRate(ZERO_ADDRESS, "997600000000000000")
